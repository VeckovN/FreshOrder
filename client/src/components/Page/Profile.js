...
//#endregion StatesHandlers
    ...
    // const usernameInput = (commitObject) =>{
    //     if(usernameShow){
    //         if(username!='')
    //         {
    //             //!!!! CHECK username INPUT (Without signs @%!!@@8 and other unwanted words)
    //             let validateObject = {'username': username};
    //             if(validateField(validateObject)){
    //                 commitObject.username = username;
    //                 usernameError && setUsernameError('');
    //                 console.log("OBJECT" + commitObject);
    //             }
    //             else{
    //                 setUsernameError('No numbers and signs');
    //             }
    ...
    let profileContext =   
        <div className='input_container'>
            <div className='input_field'>
                ...
                {usernameShow && <input onChange={usernameChangeHandler} id='username' type='text'/>}
                {/* {usernameError && <div className='errorInput'>{usernameError}</div>} */}
                {usernameError && <label className='errorInput'>{usernameError}</label>}
            </div>

            <div className='input_field'>
                <div  onClick={emailShowHandler} >Email: <span>{userInfo.email}</span></div>
                {emailShow && <input onChange={emailChangeHandler} type='text'></input>}
                {emailError && <label className='errorInput'>{emailError}</label>}
            </div>

            <div className='input_field'>
                <div  onClick={phoneNumberShowHandler}>PhoneNumber: <span>{userInfo.phone_number}</span></div>
                {phoneNumberShow && <input onChange={phoneNumberChangeHandler} type='number'></input>}
                {phoneNumberError && <label className='errorInput'>{phoneNumberError}</label>}
            </div>

            <div className='input_field'>
                <div onClick={addressShowHandler}>Address: <span>{userInfo.address}</span></div>
                {addressShow && <input onChange={addressChangeHandler}  type='text'></input>}
                {addressError && <label className='errorInput'>{addressError}</label>}
            </div>
            ...
            <div className='input_field'>
                ...
                {passwordShow && 
                <div>
                    <input onChange={passwordChangeHandler} type='password'></input> 
                    {passwordError && <label className='errorInput'>{passwordError}</label>}
                    {/* //Only if password input exist then show repeatPassword */}
                    <div>Repeat Password</div>
                    <input onChange={repeatPasswordChangeHandler} type='password'></input>
                    {repeatPasswordError && <label className='errorInput'>{repeatPasswordError}</label>}
                </div>}
            </div>
